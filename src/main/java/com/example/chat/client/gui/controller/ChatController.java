...
public class ChatController {
    ...
    private void handleJoinRoomSuccess(Message message) {
        ...
        Platform.runLater(() -> {
            ...
            List<ChatMessage> messages = roomMessages.get(currentRoom);
            if (messages != null) {
    ...
    private void handleRoomMessage(Message message) {
        if (message.getRoomName().equals(currentRoom)) {
            ...
            // 保存消息
            roomMessages.computeIfAbsent(currentRoom, k -> new ArrayList<>()).add(chatMessage);
            
            Platform.runLater(() -> addMessageToDisplay(chatMessage));
        }
    }
    ...
    private void handleUserJoinedRoom(Message message) {
        if (message.getRoomName().equals(currentRoom)) {
            ChatMessage systemMessage = new ChatMessage(
                "系统", message.getSender() + " 加入了房间", message.getTimestamp(), true);
            
            roomMessages.computeIfAbsent(currentRoom, k -> new ArrayList<>()).add(systemMessage);
            Platform.runLater(() -> addMessageToDisplay(systemMessage));
        }
    }
    ...
    private void handleUserLeftRoom(Message message) {
        if (message.getRoomName().equals(currentRoom)) {
            ChatMessage systemMessage = new ChatMessage(
                "系统", message.getSender() + " 离开了房间", message.getTimestamp(), true);
    ...
    private void addMessageToDisplay(ChatMessage message) {
        ...
        if (message.isSystem) {
            senderText.setStyle("-fx-fill: #ff6b35; -fx-font-weight: bold; -fx-font-family: '" + FONT_FAMILY + "'; -fx-font-size: 14px;");
        } else {
            senderText.setStyle("-fx-fill: #333; -fx-font-weight: bold; -fx-font-family: '" + FONT_FAMILY + "'; -fx-font-size: 14px;");
        }
